...
class QuestionsAdapter(
    ...
    ListAdapter<AnswerData, RecyclerView.ViewHolder>(object : DiffUtil.ItemCallback<AnswerData>() {
        override fun areItemsTheSame(oldItem: AnswerData, newItem: AnswerData): Boolean = oldItem.questionId == newItem.questionId
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): RecyclerView.ViewHolder = when (QuestionType.values()[viewType]) {
        QuestionType.TEXT, QuestionType.NUMBER, QuestionType.NUMBER_DECIMAL -> QuestionTextAdapterItemView(
            ItemQuestionTextBinding.inflate(parent.context.layoutInflater, parent, false)
        )
        QuestionType.MULTIPLE_CHOICE -> QuestionMultipleChoiceAdapterItemView(
            ItemQuestionMultipleChoiceBinding.inflate(parent.context.layoutInflater, parent, false)
        )
        QuestionType.SINGLE_CHOICE -> QuestionSingleChoiceAdapterItemView(
    ...
    private inner class QuestionTextAdapterItemView(
        ...
        fun bind(question: AnswerData) = with(binding) {
            questionName.setText(question.question)
            questionValue.inputType = when (question.type) {
    ...
    private inner class QuestionMultipleChoiceAdapterItemView(
        ...
        fun bind(question: AnswerData) = with(binding) {
            ...
            val checkBoxes = question.possibleAnswers?.map { answer ->
    ...
    private inner class QuestionSingleChoiceAdapterItemView(
        ...
        fun bind(question: AnswerData) = with(binding) {
            ...
            val radioButtons = question.possibleAnswers?.map { answer ->
                val radioButton = MaterialRadioButton(root.context).apply {
                    setTextAppearance(R.style.Comfortly_BodyLarge_Medium)
                    layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT)
                    setText(answer)
